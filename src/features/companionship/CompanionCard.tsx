    ...
    <section className={`surface companion-card relative mt-3 overflow-hidden rounded-2xl p-4 sm:p-5 ${sharedToday ? 'companion-bloom' : ''}`} aria-labelledby="companion-title" aria-busy={data.refreshing}>
      ...
      <div className="mt-4 grid grid-cols-7 gap-1.5" aria-label="最近七天共同绽放记录">
        {lastSevenDates.map((date) => {
          ...
          return <span key={date} className={`grid min-h-9 min-w-0 place-items-center rounded-lg ${bloomed ? 'bg-[var(--accent-soft)] text-[var(--accent-strong)]' : 'bg-[var(--surface-soft)] text-[var(--muted)]'}`} title={`${date}${bloomed ? '共同绽放' : '安静记录'}`}><Flower2 size={bloomed ? 18 : 14} strokeWidth={bloomed ? 2 : 1.5} aria-hidden="true" /></span>
        })}
      </div>

      <div className="mt-4 flex min-w-0 flex-wrap items-center justify-between gap-3 border-t border-[var(--line)] pt-4">
        <div className="min-w-0"><p className="text-sm font-semibold">本周一起学习了 {state.weekBloomDays} 天</p>{receivedToday && <p className="mt-0.5 text-xs text-[var(--rose)]">{mutualToday ? '今天你们互相送了一朵花' : `${state.primaryCompanionName} 今天送来一朵花`}</p>}</div>

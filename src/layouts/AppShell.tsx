  ...
  return (
    <div className="app-background min-h-[100dvh]">
      ...
      <nav className="safe-bottom fixed inset-x-0 bottom-0 z-30 grid grid-cols-3 border-t border-[var(--line)] bg-[color-mix(in_srgb,var(--surface)_94%,transparent)] px-3 pt-2 backdrop-blur-xl md:hidden" aria-label="主导航">
        {items.map(({ to, label, icon: Icon }) => <NavLink key={to} to={to} className={({ isActive }) => `focus-ring grid min-h-14 justify-items-center gap-1 rounded-xl py-1 text-xs font-semibold ${isActive ? 'text-[var(--accent-strong)]' : 'text-[var(--muted)]'}`}><Icon size={21} strokeWidth={1.8} />{label}</NavLink>)}
      </nav>
      <InstallPWA />
    </div>
  )

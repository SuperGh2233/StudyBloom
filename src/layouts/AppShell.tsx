      ...
      <StudySessionProvider>
        ...
        <header className="pwa-safe-top sticky top-0 z-30 border-b border-[var(--line)] bg-[color-mix(in_srgb,var(--page)_90%,transparent)] backdrop-blur-xl">
          <div className="pwa-safe-inline mx-auto flex h-16 max-w-7xl min-w-0 items-center justify-between gap-2">
            <NavLink to="/calendar" className="focus-ring flex min-w-0 items-center gap-2 rounded-xl" aria-label="StudyBloom 日历首页">
              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[linear-gradient(140deg,var(--accent)_0%,var(--accent-strong)_70%)] text-white shadow-[0_4px_12px_-4px_var(--accent-strong)]"><Sprout size={19} strokeWidth={1.9} /></span>
        ...
        <nav className="safe-bottom pwa-safe-inline fixed inset-x-0 bottom-0 z-30 grid grid-cols-5 border-t border-[var(--line)] bg-[color-mix(in_srgb,var(--surface)_94%,transparent)] pt-2 backdrop-blur-xl md:hidden" aria-label="主导航">
          {items.map(({ to, label, icon: Icon }) => (
            <NavLink key={to} to={to} className={({ isActive }) => `focus-ring grid min-h-14 min-w-0 justify-items-center gap-0.5 rounded-xl py-1 text-[11px] font-semibold transition ${isActive ? 'text-[var(--accent-strong)]' : 'text-[var(--muted)]'}`}>
              {({ isActive }) => (
                <>
                  <span className={`grid h-7 w-12 place-items-center rounded-full transition ${isActive ? 'bg-[var(--accent-soft)] shadow-[inset_0_0_0_1px_color-mix(in_srgb,var(--accent)_30%,transparent)]' : ''}`}>
                    <Icon size={20} strokeWidth={isActive ? 2 : 1.8} />
                  </span>
                  <span className="max-w-full min-w-0 truncate">{label}</span>
                </>
              )}
            </NavLink>
          ))}

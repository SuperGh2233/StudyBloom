...
export function CalendarPage() {
  ...
  return (
    <div className="gentle-enter min-w-0">
      <header className="mb-4 grid min-w-0 gap-3 sm:mb-5 sm:gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
        <div className="min-w-0">
          <p className="mb-1 flex items-center gap-2 text-xs font-semibold text-[var(--accent-strong)] sm:text-sm"><Leaf size={16} />本月计划</p>
          <h1 className="truncate text-2xl font-bold tracking-[-0.03em] sm:text-4xl">{format(month, 'yyyy年M月', { locale: zhCN })}</h1>
          <p className="mt-1.5 text-sm text-[var(--muted)]">完成 {completed}/{data.tasks.length} 项，本月完成率 {rate}%</p>
        </div>

...
> Supabase 需要按文件名顺序执行 `migrations/` 中的全部迁移。已有 V0.8.1 项目升级时至少继续执行 `20260816000000_add_companionship.sql`（步骤见 [supabase/README.md](supabase/README.md)），否则“一起绽放”会显示加载错误。
...
- 搭子概要只通过 `get_companion_summary` 安全聚合函数返回最近七天数据。好友不会获得 `study_sessions` 或 `study_session_segments` 的读取权限；`bloom_only` 只返回是否达到有效学习条件，`summary` 才返回整数分钟和任务完成数量。

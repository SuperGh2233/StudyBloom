...
-- V0.7.0: one personal countdown and private per-owner friend remarks.
alter table public.study_preferences
...
create trigger friend_notes_set_updated_at before update on public.friend_notes
for each row execute function public.set_updated_at();
...
create trigger friendships_cleanup_notes after update or delete on public.friendships
...
create policy friend_notes_select_own on public.friend_notes for select using (auth.uid() = owner_id);
...
create policy friend_notes_delete_own on public.friend_notes for delete using (auth.uid() = owner_id);
